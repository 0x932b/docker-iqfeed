...
# Step 1: Bootstrap APT, fix GPG keys, and perform initial upgrade
RUN dpkg --add-architecture i386 && \
    # Temporarily trust repos to install gnupg (which provides the 'gpg' command)
    sed -i '/^Types: deb$/a Trusted: yes' /etc/apt/sources.list.d/ubuntu.sources && \
    apt-get update && \
    apt-get install -yq --no-install-recommends gnupg wget && \
    # Now, use wget and gpg to add the missing key properly
    wget -qO- "https://keyserver.ubuntu.com/pks/lookup?op=get&search=0x871920D1991BC93C" | gpg --dearmor -o /usr/share/keyrings/ubuntu-archive-keyring-2024.gpg && \
    # Remove the temporary trust setting
    sed -i '/^Trusted: yes/d' /etc/apt/sources.list.d/ubuntu.sources && \
    # Run a final, clean update and upgrade to verify the key and get a stable base
    apt-get update && \
    apt-get upgrade -yq
...
RUN apt-get install -yq --no-install-recommends \
        ...
        bbe netcat-openbsd net-tools git python3 python3-setuptools \
        python3-numpy python3-pip python3-tz python3-psycopg2 \
        python3-dateutil python3-sqlalchemy python3-pandas && \
    # Cleaning up in the same layer to reduce image size
